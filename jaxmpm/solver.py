...
def build_solver(cfg, n_particles):
    ...
    def substep(carry, xs):
        step_idx, band_raw = xs
        # Ensure positivity via softplus, fix last band to zero
        ...
        p_rho, v, x, C, pressure, x_hist = carry
        ...
        x_hist = x_hist.at[step_idx].set(x)
        return (p_rho, v, x, C, pressure, x_hist), None
    ...
    def simulate(p_rho, v, x, C, pressure, friction_bands_raw):
        ...
        where ``x_history`` has been sub-sampled every ``save_every`` steps.
        """
        x_hist = jnp.zeros((n_steps + 1, n_particles, 2))
        x_hist = x_hist.at[0].set(x)
        carry = (p_rho, v, x, C, pressure, x_hist)
        ...
        p_rho, v, x, C, pressure, x_hist = carry
        return p_rho, v, x, C, pressure, x_hist[::save_every]
